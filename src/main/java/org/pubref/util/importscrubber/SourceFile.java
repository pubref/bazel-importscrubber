...
public class SourceFile {
  ...
  public SourceFile(File file, String encoding) throws IOException {
    ...
    while ((currentLine = buff.readLine()) != null) {
      // discard imports
      if (currentLine.startsWith(ImportStatements.MARKER) && !isStaticImport(currentLine)) {
  ...
  public void addImport(String className) {
    try {
      ...
      System.out.println(
          "Non-fatal, but unexpected error while processing " + className + " in " + _file + " --");
      e.printStackTrace();
    }
  }

  public void save(StatementFormat format) throws IOException {
    if (ImportScrubber.DEBUG) System.out.println("Imports before pruning:" + _imports.getCount());
