...
public class ImportStatements {
  ...
  public void removeInnerClasses(String className) {
    if (ImportScrubber.DEBUG) System.out.println("Looking for inner classes of " + className);
    for (Iterator i = stmts.iterator(); i.hasNext(); ) {
      ImportStatement stmt = (ImportStatement) i.next();
  ...
  public String removeUnreferenced(String classBody) {
    for (Iterator i = stmts.iterator(); i.hasNext(); ) {
      ...
      if (j != -1) {
        if (ImportScrubber.DEBUG) System.out.println("FQ class found:" + stmt.getClassName());
        ...
        if (bareClass) {
          if (ImportScrubber.DEBUG) System.out.println("Bare class also found");
          for (Iterator k = stmts.iterator(); k.hasNext(); ) {
            ...
            if (s2 != stmt && stmt.getClassName().compareTo(s2.getClassName()) == 0) {
              return "ambiguous use of "
                  + stmt.getClassName()
                  + "."
                  + ImportScrubber.LINE_SEPARATOR
                  + "\t("
                  + stmt.getFullyQualifiedClassName()
                  + " and "
                  + s2.getFullyQualifiedClassName()
                  + ")"
                  + ImportScrubber.LINE_SEPARATOR
          ...
          if (ImportScrubber.DEBUG) System.out.println("No bare class found; removing");
          i.remove();
        }
      }
    }
